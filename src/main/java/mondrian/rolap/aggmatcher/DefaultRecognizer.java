...
class DefaultRecognizer extends Recognizer {
     ...
     */
    protected Matcher getIgnoreMatcher() {
     ...
     * column.
     */
    protected Matcher getFactCountMatcher() {
     ...
     */
    protected Matcher getMeasureMatcher(
        Column.Usage factUsage)
    {
     ...
     */
    protected int matchForeignKey(Column.Usage factUsage) {
        Column factColumn = factUsage.getColumn();
        ...
        int matchCount = 0;
        for (Column aggColumn : aggTable.getColumns()) {
            // if marked as ignore, then do not consider
            if (aggColumn.hasUsage(JdbcSchema.UsageType.IGNORE)) {
                continue;
            }
        ...
        final HierarchyUsage hierarchyUsage)
    {
        ...
        try {
            List<Pair<RolapLevel, Column>> levelMatches =
                new ArrayList<Pair<RolapLevel, Column>>();
            level_loop:
            for (Level level : hierarchy.getLevels()) {
                ...
                Matcher matcher = getRules().getLevelMatcher(
            ...
            Collections.sort(
                levelMatches,
                new Comparator<Pair<RolapLevel, Column>>() {
                    public int compare(
            ...
            for (Pair<RolapLevel, Column> pair
                : levelMatches)
            {
