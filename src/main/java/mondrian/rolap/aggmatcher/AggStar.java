...
public class AggStar {
        ...
        final int approxRowCount)
    {
        ...
        // 1. load fact count
        for (Iterator<Usage> it =
             dbTable.getColumnUsages(UsageType.FACT_COUNT);
             it.hasNext();)
        {
            Usage usage = it.next();
        ...
        // 2. load measures
        for (Iterator<Usage> it =
             dbTable.getColumnUsages(UsageType.MEASURE);
             it.hasNext();)
        {
            Usage usage = it.next();
        ...
        // 3. load foreign keys
        for (Iterator<Usage> it =
             dbTable.getColumnUsages(UsageType.FOREIGN_KEY);
             it.hasNext();)
        {
            Usage usage = it.next();
        ...
        // 4. load levels
        for (Iterator<Usage> it =
             dbTable.getColumnUsages(UsageType.LEVEL);
             it.hasNext();)
        {
            Usage usage = it.next();
     ...
     */
    private final Map<Integer, Table.Column> levelColumnsToJoin;
        ...
        final int approxRowCount)
    {
        ...
        this.levelColumnsToJoin = new HashMap<Integer, Table.Column>();
    }
     ...
     */
    public FactTable.Measure lookupMeasure(final int bitPos) {
        Table.Column column = lookupColumn(bitPos);
        return (column instanceof FactTable.Measure)
            ?  (FactTable.Measure) column
            : null;
    }
    /**
     ...
     */
    public Table.Level lookupLevel(final int bitPos) {
        Table.Column column = lookupColumn(bitPos);
        return (column instanceof FactTable.Level)
            ?  (FactTable.Level) column
            : null;
    }
     ...
     * the array of columns.
     */
    public Table.Column lookupColumn(final int bitPos) {
     ...
     */
    private void addColumn(final Table.Column column) {
        columns[column.getBitPosition()] = column;
    }

    private static final Logger JOIN_CONDITION_LOGGER =
            Logger.getLogger(Table.JoinCondition.class);
    ...
    public abstract class Table {
         ...
         * Add a Level column.
         */
        protected void addLevel(final Level level) {
        ...
        /**
         * Converts a {@link RolapStar.Table} into a
         * {@link DimTable} as well as converting all columns and
         ...
         */
        protected DimTable convertTable(
...
*/
    ...
    public class FactTable extends Table {
         ...
         */
        private void loadForeignKey(final Usage usage) {
         ...
         */
        private void loadMeasure(final Usage usage) {
            ...
            MondrianDef.Expression expression;
            if (column.hasUsage(UsageType.FOREIGN_KEY)
         ...
         */
        private void loadFactCount(final Usage usage) {
         ...
         */
        private void loadLevel(final Usage usage) {
