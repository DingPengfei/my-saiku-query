 ...
 *
 * <p>Unlike {@link BitSet}, the number of bits cannot be changed
        ...
        extends Serializable, Comparable<BitKey>, Iterable<Integer>
{
    ...
    abstract class AbstractBitKey implements BitKey {
         ...
         *
         * <p>(Copied from {@link Long#bitCount(long)}, which was
    ...
    public class Small extends AbstractBitKey {
        ...
        public BitKey or(BitKey bitKey) {
            if (bitKey instanceof Small) {
                final Small other = (Small) bitKey;
                final Small bk = (Small) copy();
                bk.or(other.bits);
                return bk;

            } else if (bitKey instanceof Mid128) {
                final Mid128 other = (Mid128) bitKey;
                final Mid128 bk = (Mid128) other.copy();
                bk.or(this.bits, 0);
                return bk;

            } else if (bitKey instanceof Big) {
                final Big other = (Big) bitKey;
                final Big bk = (Big) other.copy();
                bk.or(this.bits);
                return bk;
            }
     ...
     * {@link BitSet}, but does not require dynamic resizing.
     */
    public class Big extends AbstractBitKey {
        ...
        public BitKey and(BitKey bitKey) {
            if (bitKey instanceof Small) {
                final Small bk = (Small) bitKey.copy();
                bk.and(bits[0]);
                return bk;

            } else if (bitKey instanceof Mid128) {
                final Mid128 bk = (Mid128) bitKey.copy();
